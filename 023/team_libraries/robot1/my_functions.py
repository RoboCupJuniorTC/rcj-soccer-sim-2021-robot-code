...
def artilheiro(self, data, STOP_NEAR_GOALKEEPER, KICK_FLAG, kick_intensity) -> int:
    ...
    dist2goleiro = get_distBetweenObjects(robot_pos, data[MY_ROBOT_NAMES[min_arg]]) # distancia entre o artilheiro e o goleiro
    dist_ball2goleiro = get_distBetweenObjects(data['ball'], data[MY_ROBOT_NAMES[min_arg]]) # distancia entre a bola e o goleiro
    ...
    if dist2goleiro < 0.2 and dist_ball2goleiro < 0.1 and STOP_NEAR_GOALKEEPER:
        ...
        if dist_ball2robot <= 0.065:
            if INVERT_SIDE == -1:
                ...
                kick_rotation, d_angle, d_angle_own = get_angle_Goal2Robot(robot_pos, robot_angle, OWN_GOAL, OP_GOAL, INVERT_SIDE)
            else:
                # Get the info about rotation kick and the angle between robot's front and opponent's goal               
                kick_rotation, d_angle, d_angle_own = get_angle_Goal2Robot(robot_pos, robot_angle, OP_GOAL, OWN_GOAL, INVERT_SIDE)
            if KICK_FLAG == 0:  
                if kick_rotation == 0:
...
def condition_for_banheira2(self, data) -> bool:
    if self.name[0] == 'Y':
        my_goal_thres =  -0.65
        if more_than_one_adversary(self, data, my_goal_thres) and data['ball']['x'] < my_goal_thres and are_ball_near_GK(self, data):
            return True
    else:
        my_goal_thres =  0.65
        if more_than_one_adversary(self, data, my_goal_thres) and data['ball']['x'] > my_goal_thres and are_ball_near_GK(self, data):
...
def banheira2(self, data, game_clock, CONTANDO_FLAG, BALL_AREA, INICIO, ASSISTENCIA, SUPORTE_GOLEIRO) -> Tuple[bool, float, bool]:
...
def condition2artilheiro(self, data):
    ...
    if (data['ball']['y'] < 0 and data[self.name]['y'] > data[GK]['y']) or (data['ball']['y'] > 0 and data[self.name]['y'] < data[GK]['y']):
            return True
