...
from team_023_libraries.robot1.rcj_soccer_robot import RCJSoccerRobot, TIME_STEP

from team_023_libraries.robot1 import my_functions
#-----------------------------------------------------------------------------------------
...
class MyRobot(RCJSoccerRobot):
    def run(self, counter, score, goal_time, GOAL_FLAG, TIME_FLAG, IMPORT_FLAG, KICK_FLAG, CONTANDO_FLAG, BALL_AREA, INICIO):     
        while self.robot.step(TIME_STEP) != -1:
            if self.is_new_data():
                data = self.get_new_data()
                #--------------------------------------------------- INITS CRIADAS POR ARMADILLOS
                game_clock, counter = my_functions.game_time (counter)
                score, goal_time, GOAL_FLAG, TIME_FLAG = my_functions.scoreboard (data, score, game_clock, goal_time,GOAL_FLAG, TIME_FLAG)
                ...
                TRUNCAMENTO_FLAG, CONTANDO_FLAG, BALL_AREA, INICIO = my_functions.truncamento (data,game_clock, CONTANDO_FLAG, BALL_AREA, INICIO)
                ...
                if (mais_proximo_do_gol == True and mais_proximo_da_bola == True):
                    if (jogador_mais_proximo == False and mais_proximo_da_bola == False and adv_mais_proximo == True):
                        ...
                        KICK_FLAG = my_functions.artilheiro(self, data, STOP_NEAR_GOALKEEPER, KICK_FLAG, KICK_INTENSITY_DEFAULT)
                    else:
                ...
                elif ((mais_proximo_do_gol == False and mais_proximo_da_bola == True) or (eu_sou_mais_proximo_do_gol_adv == True and mais_proximo_da_bola == True)): #verificar antes ou depois dp centro 
                    ...
                    CONTANDO_FLAG, BALL_AREA, INICIO = my_functions.banheira2(self, data, game_clock, CONTANDO_FLAG, BALL_AREA, INICIO, ASSISTENCIA, SUPORTE_GOLEIRO)                        
...
my_robot = MyRobot()
my_robot.run(counter, score, goal_time, GOAL_FLAG, TIME_FLAG, IMPORT_FLAG, KICK_FLAG, CONTANDO_FLAG, BALL_AREA, INICIO)
