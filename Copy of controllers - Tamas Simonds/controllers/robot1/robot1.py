...
class Movement:
    ...
    def moveToPoint(self,robotPos, robot_angle, dest):
        ...
        #print("Dis", detector.getDisBetweenPoints(robotPos, dest))
...
class PlayController:
    ...
    resetBallRobotPos = Position(0.2,0)
    ballStationaryCount = 0
    realignInCenter = False
    centerRealignConter = 0
    def update(self, robotPos, robotAngle,ballPos):
        ...
        if ballPos.x  > 0.1 and switchSidesMulti == 1 or ballPos.x < -0.1 and switchSidesMulti == -1:
            print("DEFENCE")
            #Defence
            goalLine = Position(0.7,0)

            disFromGoals = detector.getVerticleDis(goalLine, robotPos)

            # if disFromGoals < 0.15:
            #     movement.moveToBall(robotPos, robotAngle, Position(goalLine.x,ballPos.y))
            # else:
            if robotPos.x > 0.7:
                movement.followPoint(robotPos, robotAngle, Position(robotPos.x * switchSidesMulti, ballPos.y))
            else:
                movement.followPoint(robotPos, robotAngle, Position(ballPos.x * switchSidesMulti, ballPos.y))
            if detector.getDisBetweenPoints(ballPos, robotPos) < 0.2 and robotPos.x > ballPos.x and robotPos.x > 0.7:
                print("TRIGGERED")
                movement.moveToBall(robotPos, robotAngle, ballPos)


            #If Ball Is stationary for 10 seconds run this code
            if self.isBallGoingtoBeReset == True:
            ...
            #Offence
            #Chills at half way and move to where ball will intersect half way
            print("OFFENCE")
            ...
            #print(ballPos.y)
            print("DEST",Position(0,ballPos.y))
            disToHalfWayLine = detector.gethorizontalDis(Position(0,0), robotPos)
            disHorzToBall = detector.gethorizontalDis(robotPos,ballPos)
            ballDisToHalWay = detector.gethorizontalDis(Position(0,0), ballPos)
            if ballDisToHalWay < 0.4 and ballPos.x < robotPos.x * switchSidesMulti:
                movement.moveToBall(robotPos,robotAngle,ballPos)

            else:
                if self.ballStationaryCount > 30:
                    movement.moveToPoint(robotPos,robotAngle, self.resetBallRobotPos)
                else:
                    des = ballMotionPredictor.getPointOfIntWithLine(Position(0,0))
                    #movement.followPoint(robotPos, robotAngle, des)
                    if abs(robotPos.x) < 0.2 and self.realignInCenter == False:
                        movement.followPoint(robotPos,robotAngle,Position(robotPos.x * switchSidesMulti,ballPos.y))
                    else:
                        self.realignInCenter = True
                        self.centerRealignConter += 1
                        if self.centerRealignConter > 30:
                            self.realignInCenter = False
                            self.centerRealignConter = 0
                        movement.followPoint(robotPos, robotAngle, Position(0, ballPos.y))
