...
class RCJSoccerRobotT:
    ...
    def get_angles(self, ball_pos: dict, robot_pos: dict, waiting_pos: dict) -> Tuple[float, float, float]:
        """"
        ...
        #skusam spravit angle na waiting pos
        waiting_angle = math.atan2(
            waiting_pos['y'] - robot_pos['y'],
            waiting_pos['x'] - robot_pos['x'],
        )
        #premienaju na kladne  lebo 2Pi je 360 stupnov a math.degrees to zmeni na stupne 
        if angle < 0:
        ...
        if waiting_angle < 0:
            waiting_angle = 2 * math.pi + waiting_angle

        robot_ball_angle = math.degrees(angle + robot_angle)
        robot_waiting_angle = math.degrees(waiting_angle + robot_angle)
        ...
        robot_waiting_angle -= 90
        if robot_waiting_angle > 360:
            robot_waiting_angle -= 360

        return robot_ball_angle, robot_angle, robot_waiting_angle,   
