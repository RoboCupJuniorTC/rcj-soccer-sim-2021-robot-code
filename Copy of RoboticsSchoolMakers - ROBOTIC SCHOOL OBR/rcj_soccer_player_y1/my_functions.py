...
def theChoosenOne(self, data) -> Tuple[bool, float]:
    if self.name[0] =='Y':
...
def theChoosenGoalKeeper(self, data) -> float:
    if self.name[0] =='Y':
        MY_ROBOT_NAME = ["Y1", "Y2", "Y3"]
        OWN_GOAL = [-0.856, 0]
    else:
...
def artilheiro(self, data, STOP_NEAR_GOALKEEPER, KICK_FLAG, kick_intensity) -> int:
    ...
    dist2goleiro = get_distBetweenObjects(robot_pos, data[MY_ROBOT_NAMES[0]]) # distancia entre o artilheiro e o goleiro
    dist_ball2goleiro = get_distBetweenObjects(data['ball'], data[MY_ROBOT_NAMES[0]]) # distancia entre a bola e o goleiro
...
def goleiro(self, data, distancia_x, distancia_y):
...
def lookAngle(self, data, angle) -> bool:
...
def banheira(self, data, point, look, angle):
    ...
    if dist_robot2point < 0.05: 
        ...
        if look == "ball":
            is_forward = face_the_ball(self, data)

        elif look == "angle":
            is_forward = lookAngle(self, data, angle)
        else:
