            ...
            if self.is_new_data():
                ...
                if flag == 1:
                    ...
                    if(abs(ball_pos["x"]) < 0.01 and abs(ball_pos["y"]) < 0.01):
                        ...
                        speed = 0
                    if(utils.norm(robot_pos["x"] - ball_pos["x"], robot_pos["y"] - ball_pos["y"]) < 0.005 or time.time() - centalTimer > 7 or utils.norm(old_ball_pos["x"] - ball_pos["x"], old_ball_pos["y"] - ball_pos["y"]) > 0.01):
                        flag = 1
